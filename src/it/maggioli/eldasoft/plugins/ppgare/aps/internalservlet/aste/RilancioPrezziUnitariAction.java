...
public class RilancioPrezziUnitariAction extends EncodedDataAction implements SessionAware {
	...
	public String view() {
		...
		if(isUserLogged()) {
			try {
				...
				if(this.vociNonSoggette == null 
				   ...
				   && this.listaRilanci != null && this.listaRilanci.size() > 0) {
					...
					if(voci != null) {						
						...
						this.session.put(
								PortGareSystemConstants.SESSION_ID_RILANCIO_PREZZI_UNITARI_NON_SOGGETTE, 
								this.vociNonSoggette);				
						this.session.put(
								PortGareSystemConstants.SESSION_ID_RILANCIO_PREZZI_UNITARI_SOGGETTE, 
								soggette);				
					}
				}
				...
				this.session.put(
						PortGareSystemConstants.SESSION_ID_RILANCIO_PREZZI_UNITARI, 
						this.vociDettaglio);				
	...
	private void calcolaTotalePrezziUnitari() throws ApsException {
		...
		this.totalePrezziUnitari = RilancioPrezziUnitariAction.getTotalePrezziUnitari(soggette);
		
		this.importoOfferto = Double.toString( 
				RilancioPrezziUnitariAction.getTotaleOfferta(this.bandiManager, codiceGara, soggette));	
	}
			...
			List<VoceDettaglioAstaType> voci) throws ApsException 
	{
		...
		return bdImportoOfferto.doubleValue(); 
	}
