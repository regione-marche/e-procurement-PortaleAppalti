...
public class ProcessPageInviaModificheAction extends EncodedDataAction implements SessionAware {
	...
	public void setComunicazioniManager(
	...
	public String add() {
		...
		try {
			...
			if (null == this.session.get(PortGareSystemConstants.SESSION_ID_CARRELLO_PRODOTTI)) {
				...
				// this.checkFileSize(this.riepilogo, 0, this.appParamManager);
				// controlliOk = controlliOk &&
				// this.checkFileName(this.riepilogoFileName);
				// controlliOk = controlliOk &&
				// this.checkFileFormat(this.riepilogo, this.riepilogoFileName,
				// PortGareSystemConstants.DOCUMENTO_FORMATO_FIRMATO);
				controlliOk = controlliOk && this.checkFileSize(this.riepilogo, this.riepilogoFileName, dimensioneRiepilogo, this.appParamManager, evento);
				controlliOk = controlliOk && this.checkFileName(this.riepilogoFileName, evento);
				controlliOk = controlliOk && this.checkFileFormat(this.riepilogo, this.riepilogoFileName, PortGareSystemConstants.DOCUMENTO_FORMATO_FIRMATO, evento, onlyP7m);
			...
			if (inviataComunicazione) {
				...
				case PortGareSystemConstants.TIPO_PROTOCOLLAZIONE_WSDM:
					...
					try {
						...
						WSDMProtocolloDocumentoType ris = this.wsdmManager.inserisciProtocollo(
								loginAttr,
								wsdmProtocolloDocumentoIn);
	...
	private WSDMProtocolloDocumentoInType creaInputProtocollazioneWSDM(
			...
			DettaglioBandoIscrizioneType bandoIscrizione) throws IOException, DocumentException 
	{
		...
		wsdmProtocolloDocumentoIn.setOggetto(this.formatI18nLabelFromDefaultLocale("LABEL_WSDM_OGGETTO_AGGPRODOTTI_CATALOGO",
		...
		if (esisteFascicolo) {
			...
			fascicolo.setOggettoFascicolo(oggetto);
			if(IWSDMManager.CODICE_SISTEMA_JDOC.equals(codiceSistema)) {
