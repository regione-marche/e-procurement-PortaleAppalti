...
<s:if test="%{#session.dettPartecipGara.tipoEvento == 1}">
	...
	<s:if test="%{#session.dettPartecipGara.rti}">
		<c:set var="codiceBalloon" value="BALLOON_PART_GARA_DETTAGLIO_RTI"/>
	</s:if>
	<s:elseif test="#session.dettPartecipGara.impresa.consorzio">
...
<s:if test="%{!#session.dettPartecipGara.editRTI && #session.dettPartecipGara.denominazioneRTIReadonly}">
