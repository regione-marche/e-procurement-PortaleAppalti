...
<s:set var="presentaPartecipazione" value="%{#session.dettPartecipGara.tipoEvento == 1}" />
<s:set var="invioOfferta" value="%{#session.dettPartecipGara.tipoEvento == 2}" />
...
<s:if test="%{#presentaPartecipazione}">
	...
	<s:if test="%{#session.dettPartecipGara.garaTelematica}">
	...
	<form action="<wp:action path="/ExtStr2/do/FrontEnd/RichPartBando/processPageRiepilogo.action" />" method="post">
		...
		<fieldset>
			...
			<div class="fieldset-row first-row">
				...
				<div class="element">
					<s:property value="%{#session.dettPartecipGara.descBando}" />
				</div>
			</div>
			...
			<div class="fieldset-row">
				...
				<div class="element">
					<s:property value="%{#session.dettPartecipGara.datiPrincipaliImpresa.ragioneSociale}" />
				</div>
			</div>
			...
			<div class="fieldset-row">
				...
				<div class="element">
					<s:if test="%{#session.dettPartecipGara.legaliRappresentantiImpresa.size() > 0}">
						<ul class="list">
							<s:iterator value="%{#session.dettPartecipGara.legaliRappresentantiImpresa.iterator()}" var="legaleRappresentante" status="stat">
			...
			<div class="fieldset-row">
				...
				<div class="element">
					<s:if test="%{#session.dettPartecipGara.direttoriTecniciImpresa.size() > 0}">
						<ul class="list">
							<s:iterator value="%{#session.dettPartecipGara.direttoriTecniciImpresa.iterator()}" var="direttoreTecnico" status="stat">
								<li class='<s:if test="%{#stat.first}">first</s:if> <s:if test="%{#stat.last}">last</s:if>'>
			...
			<div class='fieldset-row <s:if test="%{!(#session.dettPartecipGara.lottiDistinti || #session.dettPartecipGara.plicoUnicoOfferteDistinte)}">last-row</s:if>'>
				<div class="label">
					<label><wp:i18n key="LABEL_PARTECIPA_COME_MANDATARIA_RTI" /></label>
				</div>
				<div class="element">
					<s:if test="%{#session.dettPartecipGara.rti}"><wp:i18n key="LABEL_YES" />, <wp:i18n key="LABEL_PER_CONTO_DI" /> <s:property value="%{#session.dettPartecipGara.denominazioneRTI}"/></s:if>
					<s:else><wp:i18n key="LABEL_NO" /></s:else>
				</div>
			</div>

			<s:if test="%{#session.dettPartecipGara.lottiDistinti || #session.dettPartecipGara.plicoUnicoOfferteDistinte}">
				<%-- 
