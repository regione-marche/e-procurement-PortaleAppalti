...
<wp:pager listName="result" objectName="groupContent" max="10" pagerIdFromFrame="true" >
	...
	<c:if test="${groupContent.size > groupContent.max}">
		<p class="paginazione">
			...
			<c:otherwise><a href="<wp:url paramRepeat="true" ><wp:urlPar name="${groupContent.paramItemName}" ><c:out value="${groupContent.prevItem}"/></wp:urlPar></wp:url>">&laquo; <wp:i18n key="PREV" /></a></c:otherwise>					
			</c:choose>
			...
			<c:otherwise><a href="<wp:url paramRepeat="true" ><wp:urlPar name="${groupContent.paramItemName}" ><c:out value="${groupContent.nextItem}"/></wp:urlPar></wp:url>"><wp:i18n key="NEXT" /> &raquo;</a></c:otherwise>					
			</c:choose>
		</p>
	</c:if>		
	<ul>
