...
public class User extends AbstractUser implements HttpSessionBindingListener, HttpSessionActivationListener, Serializable {
	...
	/**
	 * Indica se l'utente è definito localmente all'interno del db "serv" di jAPS.
	 *
	 * @return true se è un'utente locale di jAPS, false se è un'utente definito
	 * in altra base dati.
	 */
	...
	public void valueUnbound(HttpSessionBindingEvent hsbe) {
		if (!this.getUsername().equals(SystemConstants.GUEST_USER_NAME)) {
			...
			try {
				...
				manager.logLogout(this.getUsername(), this.getIpAddress());
			} catch (Throwable e) {
				evento.setError(e);
	...
	public void sessionDidActivate(HttpSessionEvent event) {
		...
		if (datiUtente == null && !SystemConstants.GUEST_USER_NAME.equals(this.getUsername())) {
			...
			trackerSessioni.putSessioneUtente(event.getSession(), this.getIpAddress(), this.getUsername(), DateFormatUtils.format(new Date(event.getSession().getCreationTime()), "dd/MM/yyyy HH:mm:ss"));
		}
	}
