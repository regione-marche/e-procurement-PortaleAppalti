...
public class EsitiFinderAction extends EncodedDataAction implements
	...
	private String getListaEsiti(String fromPage) {
		...
		try {
			...
			this.addActionErrorDateInvalid("LABEL_DATA_PUBBLICAZIONE_ESITO", DA_DATA, this.model.getDataPubblicazioneDa());
		...
		try {
			...
			ApsSystemUtils.logThrowable(e, this, fromPage);
			this.addActionErrorDateInvalid("LABEL_DATA_PUBBLICAZIONE_ESITO", A_DATA, this.model.getDataPubblicazioneA());
		...
		if (SUCCESS.equals(this.getTarget()) && dateOk) {
			try {
				...
				if(fromPage.equalsIgnoreCase(LIST_ALL_IN_CORSO)) {
					this.listaEsiti = this.esitiManager.getElencoEsiti(
				...
				if(fromPage.equalsIgnoreCase(LIST_ALL_AFFIDAMENTI)) {
					this.listaEsiti = this.esitiManager.getElencoEsitiAffidamenti(
							this.model.getStazioneAppaltante(),
							this.model.getOggetto(), 
							this.model.getCig(),
							this.model.getTipoAppalto(), 
				...
				if(fromPage.equalsIgnoreCase(SEARCH)) {
					this.listaEsiti = this.esitiManager.searchEsiti(
							this.model.getStazioneAppaltante(),
							this.model.getOggetto(), 
							this.model.getCig(),
							this.model.getTipoAppalto(), 
