...
public class RegistrazioneImpresaManager extends UserRegManager implements
    ...
    public EsitoOutType registerImpresa(
    		String username, 
			...
			boolean isVatGroup) 
    {
		...
		try {
			...
			if (risultato.isEsitoOk() && partitaIVA != null && this._datiImpresaChecker.existsPartitaIVA(partitaIVA) && !isVatGroup) {
				risultato.setEsitoOk(false);
			...
			if (risultato.isEsitoOk()) {
				...
				this._regImpresaProfileManager.addProfile(username, regImpresaProfile);
			}
	...
	public EsitoOutType activateImpresa(String username, String denominazione) {
		...
		try {
		    ...
		    if (risultato.isEsitoOk()) {
				 ...
				 * come utente delegato per l'impresa) il token non dovr� essere generato
				 * */
	...
	public EsitoOutType sendActivationMailImpresa(String username, String email) {
		...
		try {
			...
			if (risultato.isEsitoOk()) {
				 ...
				 * 	- un'impresa si registri autonomamente al portale (non avr� un delegateUser associato) => verr� generato il token di attivazione da inserire all'interno della mail di registrazione
				 * 	- un soggetto fisico operi per conto di un operatore economico => non verr� generato alcuno token e si andr� a sfruttare l'operazione di riattivazione
				 * */
