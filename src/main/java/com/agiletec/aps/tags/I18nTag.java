...
public class I18nTag extends TagSupport {
	...
	private void extractLabel(Lang currentLang) throws ApsSystemException {
		...
		try {
			if (StringUtils.isEmpty(_lang) || currentLang.getCode().equalsIgnoreCase(_lang)) {
				_label = geti18nLabel(i18nManager, _key, currentLang.getCode());
				if (_label == null) {
					...
					_label = geti18nLabel(i18nManager, _key, defaultLang.getCode());
				}
			} else
				_label = geti18nLabel(i18nManager, _key, _lang);
			if (_label == null)
				_label = _key;
		} catch (Throwable t) {
	...
	/**
	 * Ritorna la label dati manager, chiave e lingua. In caso sia specificato, non printa i log.
	 *
	 * @param i18nManager
	 * @param key
	 * @param lang
	 ...
	 */
	private String geti18nLabel(II18nManager i18nManager, String key, String lang) throws ApsSystemException {
		return _disableLog != null
			   ? i18nManager.getLabel(key, lang, !_disableLog)
			   : i18nManager.getLabel(key, lang);
	}
	...
	public void setDisableLog(Boolean _disableLog) {
		this._disableLog = _disableLog;
	}
	...
	private Boolean _disableLog = false;
