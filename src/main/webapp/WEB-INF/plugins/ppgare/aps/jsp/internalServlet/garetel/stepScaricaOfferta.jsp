...
<s:set var="helper" value="%{#session['dettaglioOffertaGara'].bustaEconomica.helper}" />
						...
						<td class="azioni">
							<s:if test="%{#status.index==0}">
								<s:if test="%{modificaFirmatarioVisible}">
									<a href='<wp:action path="/ExtStr2/do/FrontEnd/GareTel/editFirmatarioMandataria.action"/>' class="bkg modify" title='<wp:i18n key="BUTTON_EDIT" />'>
									</a>
								</s:if>
							</s:if>
							<s:else>
								<a href='<wp:action path="/ExtStr2/do/FrontEnd/GareTel/editFirmatarioMandante.action"/>&amp;id=${status.index}' class="bkg modify" title='<wp:i18n key="BUTTON_EDIT" />'>
								</a>
							</s:else>
						</td>
					</tr>
