...
public abstract class GenPDFAction extends EncodedDataAction implements SessionAware, IDownloadAction {
	...
	public String createPdf() {
		...
		if(reportInit()) {
			...
			// se subFolderTemplate è NULL o vuoto, 
			...
			String reportPath) 
	{
		...
		try {
			...
			XmlObject document = generaXmlConDatiTipizzati();
...
//				useSpecificFooter = true;
			...
			JRXmlDataSource jrxmlds = new JRXmlDataSource(
					new ByteArrayInputStream(document.toString().getBytes("UTF-8")), selectExpression);
			jrxmlds.setLocale(Locale.ITALIAN);
			jrxmlds.setDatePattern("yyyy-MM-dd");
			
			JasperPrint print = JasperFillManager.fillReport(isJasper, params, jrxmlds);
			...
			doc.getDocumentInformation().setKeywords(this.uuid);
			//doc.save(filePdf);
			...
			boolean isLiberoProfessionista) 
	{
		...
		// Rating di legalità (DL 1/2012) (19/03/2020- mod. vers. 26)
