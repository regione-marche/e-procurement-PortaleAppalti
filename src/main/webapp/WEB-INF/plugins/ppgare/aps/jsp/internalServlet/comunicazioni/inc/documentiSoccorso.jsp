								...
								<ul>
									...
									<s:iterator value="#helperDocumenti.docRichiestiId" var="idDoc" status="statusCaricato">
										<s:if test="%{#idDoc == #documento.id}">
											...
											<s:if test="%{#helperDocumenti.docRichiestiFirmaBean.get(#statusCaricato.index)!=null}">
												<s:set var="firmaWarning" value="%{!(#helperDocumenti.docRichiestiFirmaBean.get(#statusCaricato.index).firmacheck)}" />
											</s:if>
											...
											<c:choose>
												<c:when test="${skin == 'highcontrast' || skin == 'text'}">
													...
													<li>
														<a href='<wp:action path="/ExtStr2/do/FrontEnd/Comunicazioni/confirmDeleteAllegatoSoccorsoRichiesto.action"/>&amp;id=${statusCaricato.index}&amp;ext=${param.ext}&amp;codice=<s:property value="%{codice}"/>' 
...
--%>
							 <s:set var="firmaWarning" value="%{false}" />
								<s:if test="%{#helperDocumenti.docUlterioriFirmaBean.get(#status.index)!=null}">
									<s:set var="firmaWarning" value="%{!(#helperDocumenti.docUlterioriFirmaBean.get(#status.index).firmacheck)}" />
								</s:if>
								<s:if test='%{#firmaWarning}'>
									<img class="resize-svg-16" title="<wp:i18n key='LABEL_DOCUMENTO_FIRMA_NON_VERIFICATA' />" alt="<wp:i18n key='LABEL_DOCUMENTO_FIRMA_NON_VERIFICATA' />" src="<s:property value='%{#imgDocNonFirmato}'/>">
								</s:if> 
								...
								<ul>
									<c:choose>
										<c:when test="${skin == 'highcontrast' || skin == 'text'}">
											...
											<li>
												<a href='<wp:action path="/ExtStr2/do/FrontEnd/Comunicazioni/confirmDeleteAllegatoSoccorso.action"/>&amp;id=${status.index}&amp;ext=${param.ext}' 
