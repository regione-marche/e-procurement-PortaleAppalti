...
public class CohesionLoginResponseAction extends BaseAction implements
			...
			if (!"".equals(cohesionEncryptionKey)) {
				try {
					...
					if (nome != null && cognome != null && login != null) {
						...
						String ipAddress = this.getRequest().getHeader(
								"X-FORWARDED-FOR");
						if (ipAddress == null) {
						...
						TrackerSessioniUtenti.getInstance(this.getRequest().getSession().getServletContext())
								.putSessioneUtente(
						...
						this.addActionError(this.getText(
								"Errors.sso.insufficientData", new String[] {
										"Cohesion", nome, cognome, null, login,
										null, email }));
						this.getRequest().getSession()
