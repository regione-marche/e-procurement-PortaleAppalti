	...
	public String openPage() {
		...
		try {
			...
			if( (this.id != null && !this.id.isEmpty()) &&
				...
				// se in sessione sono già presenti i dettagli della 
...
//if(false) // DEBUG: salta la verifica del dettaglio già esistente in sessione  
				if( this.dettaglio != null && 
					this.id.equals(this.dettaglio.getId()) ) {
					// in sessione sono già presenti i dettagli
					// non è necessario rileggerli dal servizio 
				...
				// trovati... 1 dettaglio è il risultato atteso
				if(lista.size() <= 0) {
					this.addActionError("Impossibile visualizzare un dettaglio inesistente oppure di cui non si possiede la visibilità");
...
//						.getConfigurationValue(AppParamManager.WS_BANDI_ESITI_AVVISI_AUTHENTICATION_TOKEN));
		...
		// se in sessione sono già presenti i dettagli della comunicazione 
		...
		try {	
			...
			if(0 <= i && i < this.dettaglio.getDocumentiAllegati().length) {
				...
				// verifica se è un documento marcato/firmato (.TSD, .P7M)...
...
//					this.isfile = true;
//					this.session.put(
//							PortGareSystemConstants.SESSION_ID_DOWNLOAD_WORK_FILE_FIRMA_DIGITALE,
//							f.getAbsolutePath());
