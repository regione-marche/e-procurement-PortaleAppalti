 ...
 * @since 1.2
 */
public class ProcessPageRiepilogoImpresaAction extends BaseAction implements
	SessionAware {
			    ...
			    if (!helper.getXmlDocumentAggiornamentoAnagrafica().validate(validationOptions)) {
					...
					try {
						...
						if (esito.isEsitoOk()) {
							...
							this.addActionError(this.getText("Errors.aggiornamentoAnagraficaPortaleUnexpectedError", esito.getCodiceErrore()));
							target = "errorAgg";
						}
    ...
    public String back() {
		...
		if (helper == null) {
			...
			if (helper.isLiberoProfessionista()) {
				target = hasDatiUlteriori? "backLiberoProf": "backNoDatiUlterioriLiberoProf";
