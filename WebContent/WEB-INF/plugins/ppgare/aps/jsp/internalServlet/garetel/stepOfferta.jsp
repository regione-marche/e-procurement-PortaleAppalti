...
<div class="portgare-view">
	...
	<form action="<wp:action path="/ExtStr2/do/FrontEnd/GareTel/processPageOffTelOfferta.action" />" method="post" 
		...
		<fieldset>
			...
			<s:if test="{!#session.riepilogoBuste.listaCodiciInterniLotti.isEmpty()}">
				<div class="fieldset-row">
					...
					<div class="element">
						<s:property value="%{#session.riepilogoBuste.listaCodiciInterniLotti.get(#helper.codice)}" /> - <s:property value="%{#session.riepilogoBuste.busteEconomicheLotti.get(#helper.codice).oggetto}" />
					</div>
				</div>
			</s:if>
		...
		<fieldset>
			...
			<s:if test="%{OEPV}" >						
				<c:set var="criteriValutazione" scope="request" value="${sessionScope['offertaEconomica'].listaCriteriValutazione}"/>							
				<c:set var="criteriValutazioneEditabile" scope="request" value="${sessionScope['offertaEconomica'].criterioValutazioneEditabile}" />
