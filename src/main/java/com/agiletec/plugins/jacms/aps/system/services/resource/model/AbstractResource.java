...
public abstract class AbstractResource implements ResourceInterface, Serializable {
	...
	protected String getUrlPath(ResourceInstance instance) {
		...
		if (!Group.FREE_GROUP_NAME.equals(this.getMainGroup())) {
			...
			String DEF = "def";
			urlPath = this.getProtectedBaseURL();
			...
			if (instance.getSize() < 0) {
				...
				urlPath += instance.getSize();
			}
			urlPath += "/";
			if (instance.getLangCode() == null) {
				urlPath += DEF;
			} else {
				urlPath += instance.getLangCode();
			}
			urlPath += "/";
    	} else {
    		urlPath = this.getBaseURL();
    		if (!urlPath.endsWith("/")) urlPath += "/";
    		urlPath += this.getFolder();
    		if (!urlPath.endsWith("/")) urlPath += "/";
    		urlPath += instance.getFileName();
    	}
		return urlPath;
	}
