...
public class TestPageAction extends ApsAdminBaseTestCase {
	...
	public void testEditForCoachUser() throws Throwable {
		...
		if (null != showlet) {
			...
			assertEquals(action.isViewerPage(), showlet.getConfig().isEmpty());
			assertEquals(false, action.isViewerPage()); // ...ANCHE SE HA LA SHOWLET VIEWER!
		}
	}
	...
	public void testSearchPage() throws Throwable {
		...
		try {			
			...
			assertNotNull(action);
			assertNotNull(action.getPagesFound());
			assertEquals(14, action.getPagesFound().size());
	...
	public void testDetailPageForAdmin() throws Throwable {
		...
		assertEquals(page.getGroup(), action.getGroup());
		assertEquals(page.isShowable(), action.isShowable());
		assertEquals("Publicazione Contenuto", action.getTitles().getProperty("it"));
		assertEquals("Content Publishing", action.getTitles().getProperty("en"));
		Showlet showlet = page.getShowlets()[page.getModel().getMainFrame()];
		if (null != showlet) {
			assertEquals("content_viewer", showlet.getType().getCode());
			assertTrue(action.isViewerPage());
		}
	}
	...
	public void testAddForAdminUser() throws Throwable {
		...
		String result = this.executeAddPage(root.getCode(), "admin");
		assertEquals(Action.SUCCESS, result);
	...
	public void testAddForCustomerUser() throws Throwable {
		IPage root = this._pageManager.getRoot();
		String result = this.executeAddPage(root.getCode(), "pageManagerCustomers");
		assertEquals("pageTree", result);
		
		result = this.executeAddPage(root.getCode(), "pageManagerCustomers");
		assertEquals("pageTree", result);
		
		IPage coach_page = this._pageManager.getPage("coach_page");
		result = this.executeAddPage(coach_page.getCode(), "pageManagerCustomers");
		assertEquals("pageTree", result);
		
		IPage customers_page = this._pageManager.getPage("customers_page");
		result = this.executeAddPage(customers_page.getCode(), "pageManagerCustomers");
		assertEquals(Action.SUCCESS, result);
	...
	public void testAddForCoachUser() throws Throwable {
		IPage root = this._pageManager.getRoot();
		String result = this.executeAddPage(root.getCode(), "pageManagerCoach");
		assertEquals("pageTree", result);
		
		IPage coach_page = this._pageManager.getPage("coach_page");
		result = this.executeAddPage(coach_page.getCode(), "pageManagerCoach");
		assertEquals(Action.SUCCESS, result);
		...
		IPage customers_page = this._pageManager.getPage("customers_page");
		result = this.executeAddPage(customers_page.getCode(), "pageManagerCoach");
		assertEquals(Action.SUCCESS, result);
	...
	private String executeAddPage(String selectedPageCode, String userName) throws Throwable {
		this.setUserOnSession(userName);
		this.initAction("/do/Page", "new");
		this.addParameter("selectedNode", selectedPageCode);
		String result = this.executeAction();
	...
	public void testSavePage() throws Throwable {
		...
		try {
			...
			assertEquals("Pagina Test", addedPage.getTitles().getProperty("it"));
		} catch (Throwable t) {
			throw t;
		} finally {
			this._pageManager.deletePage(pageCode);
	...
	public void testGetBreadCrumbs() throws Throwable {
		...
		assertEquals(Action.SUCCESS, result);
		PageAction action = (PageAction) this.getAction();
		String langCode = action.getCurrentLang().getCode();
		String breadCrumbs = action.getBreadCrumbs("pagina_11", " >> ");
		if (langCode.equals("en")) {
			assertEquals("Start Page >> Page 1 >> Page 1-1", breadCrumbs);
		} else {
			assertEquals("Pagina iniziale >> Pagina 1 >> Pagina 1-1", breadCrumbs);
		}
		
		breadCrumbs = action.getBreadCrumbs("homepage", " >> ");
		if (langCode.equals("en")) {
			assertEquals("Start Page", breadCrumbs);
		} else {
			assertEquals("Pagina iniziale", breadCrumbs);
		}
		
		breadCrumbs = action.getBreadCrumbs("wrongCode", " >> ");
		assertEquals("NULL PAGE", breadCrumbs);
	}
