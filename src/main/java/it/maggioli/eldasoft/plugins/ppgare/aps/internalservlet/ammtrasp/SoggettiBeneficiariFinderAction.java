		...
		if (SUCCESS.equals(this.getTarget()) && dateOk) {
			try {
				...
				listaEsiti = this.leggeTrasparenzaManager.getProspettoBeneficiariArt18DLgs83_2012(
						...
						dtAffidamentoDa, 
						dtAffidamentoA);
				if (StringUtils.isNotEmpty(defaultDen) && CollectionUtils.isNotEmpty(listaEsiti))
					for (EsitoProspettoBeneficiariType esito : listaEsiti)
						esito.setUfficio(defaultDen);
		...
		if (SUCCESS.equals(this.getTarget())) {
			...
			for (EsitoProspettoBeneficiariType esito : this.listaEsiti) {
				...
				if (StringUtils.isNotEmpty(defaultDen))
					writer.print(StringUtilities.escapeCsv(defaultDen));
				else
