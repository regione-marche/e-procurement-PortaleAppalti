...
public class RegistrazioneImpresaAction extends BaseAction implements SessionAware {
	...
	public String send() {
		...
		if (registrazioneHelper == null) {
		    ...
		    boolean esitoCheckSintassi = registrazioneHelper.getXmlDocumentRegistrazione().validate(validationOptions);
			...
			if (!esitoCheckSintassi) {
				...
				try {
					...
					if ((ssoProtocollo == PortGareSystemConstants.SSO_PROTOCOLLO_SHIBBOLETH || ssoProtocollo == PortGareSystemConstants.SSO_PROTOCOLLO_COHESION)
							&& accountSSO == null) {
						// questo controllo e' un di piu' in quanto si suppone
						// che i problemi segnalati per Cohesion per i quali non
						// veniva settato il delegate user fossero dipesi
						// dall'accesso alla funzionalita' una volta che la
						// sessione applicativa era scaduta.
						//
						// il controllo implementato, a differenza di quello in
						// InitRegistrazioneAction, qui non può essere applicato
						// a Spid (sso.protocollo=3) in quanto tale
						// configurazione e' ibrida e pertanto potremmo arrivare
						// in questo punto della logica applicativa sia con un
						// accesso anomimo ed utilizzo della funzione
						// "Registrati" nella sezione area riservata che non
						// prevede la valorizzazione della variabile accountSSO,
						// sia per l'autenticazione single sign on SPID.
						evento.setLevel(Event.Level.ERROR);
						evento.setDetailMessage(this.getTextFromDefaultLocale("Errors.userRegistration.ssoSessionExpired"));
						...
						esito = this.registrazioneImpresaManager
								.registerImpresa(
			...
			throws Exception 
	{
		...
		try {
			...
			if(verificaManuale) {
				// verifica se esiste già la comunicazione di registrazione in stato BOZZA
				...
				if(comunicazioni != null) {
					if(comunicazioni.size() == 1) {
						...
						// ERRORE: più comunicazioni trovate o comunicazione non trovata
			...
			this.setAllegatoComunicazione(
					comunicazione,
					registrazioneHelper.getXmlDocumentRegistrazione(),
