	...
	$(document).ready(function() {
		...
		$('input[type="file"]').on("change", function() {
			...
			if( isValidFilename(file.name) ) {
			    $('div[name="myModal"]').css("display", "block")
				//document.getElementsByName('myModal').style.display="block";
				...
				$('div[name="progressmessage"]').first().html('<wp:i18n key="LABEL_FILE_UPLOADING" />');
			  	...
			  	.done(function(data) {									
					...
					$(".my-progress-bar").circularProgress({
			  		...
			  		if(data.completed == "false"){
    					...
    					$('div[name="progressmessage"]').first().html('<wp:i18n key="LABEL_FILE_UPLOADING_COMPLETING" />');
					}
...
<div name="myModal" class="modal">
  <div class="modal-content">
	...
	<div name="progressmessage" class="progressmessage"></div>
  </div>
</div>