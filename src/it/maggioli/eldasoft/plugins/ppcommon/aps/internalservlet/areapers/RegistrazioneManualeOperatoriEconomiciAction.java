...
public class RegistrazioneManualeOperatoriEconomiciAction extends EncodedDataAction implements SessionAware {
	...
	private List<FlussiInviatiKO> listaRegistazioni = new ArrayList<FlussiInviatiKO>();
...
//	
			...
			WizardDatiImpresaHelper impresa) throws ApsSystemException 
	{
		...
		String url = this.configManager.getParam(SystemConstants.PAR_APPL_BASE_URL) +
					 "it/ppgare_registr.wp?actionPath=/ExtStr2/do/FrontEnd/RegistrImpr/init.action" +
					 "&token=" + token;
		
		String text = MessageFormat.format(this.getI18nLabel("MAIL_REG_MANUALE_OP_TESTO", langCode),
										   new Object[] { motivazione, url });
