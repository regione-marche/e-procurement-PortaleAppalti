    	...
    	if(helper == null) {
    		...
    		try {
				...
				if (this.xmlImport == null) {
					...
					try {
						helper.importFromXmlPortale(this.xmlImport, this.xmlImportFileName);
		    	...
		    	} else if(OpenPageImportImpresaAction.XML_IMPORT_DGUE.equalsIgnoreCase(this.tipoImport)) {
		    		// importa da eDGUE-IT
	    			helper.importFromXmlDGUE(this.xmlImport, this.xmlImportFileName);
		    		evento.setMessage("Importazione dati impresa in formato DGUE da file " + this.xmlImportFileName + 
		    						  " per la ditta " + helper.getDatiPrincipaliImpresa().getRagioneSociale());	
		    		this.setTarget(SUCCESS);
		    	} else {
