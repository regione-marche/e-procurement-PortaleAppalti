...
//	}
	...
	private String getListaOrdini(String fromPage) {
		...
		OrderStatusEnum statoOrdine = null;
		if(StringUtils.isNotEmpty(this.model.getStato())) {
			statoOrdine = OrderStatusEnum.fromValue(this.model.getStato());
		}
...
//				}
	...
	//					filtri.setOrderStatus(OrderStatusEnum.IN_ATTESA_CONFERMA.getValue());
						...
						this.listaOrdini = this.ordiniManager.getPagedOrderListFO(
									...
									filtri);
					}else if(fromPage.equalsIgnoreCase(LIST_ORDINI_CONFERMATI)) {					
	...
	//					filtri.setOrderStatus(OrderStatusEnum.ACCETTATO_AUTOMATICAMENTE.getValue());
					...
					} else if (fromPage.equalsIgnoreCase(LIST_ORDINI_TUTTI)) {
	//					this.listaOrdini = this.ordiniManager.getPagedOrderListFO(
	//							codimp, 
	//							this.model.getCurrentPage(), 
	//							this.model.getiDisplayLength(),
	//							filtri);
						...
						//TODO confermati
						filtri.setOrderStatus(OrderStatusEnum.NUMBER_6);
						SearchResult<NsoWsOrder> tmpResult =  this.ordiniManager.getPagedOrderListFO(
								codimp, 
								this.model.getCurrentPage(), 
								this.model.getiDisplayLength(),
								filtri);
						
						this.listaOrdini.getDati().addAll(tmpResult.getDati());
						this.listaOrdini.setNumTotaleRecord(this.listaOrdini.getNumTotaleRecord()+tmpResult.getNumTotaleRecord());
						
	//					filtri.setOrderStatus(OrderStatusEnum.ACCETTATO_AUTOMATICAMENTE.getValue());
						//TODO ACCETTATO_AUTOMATICAMENTE
						filtri.setOrderStatus(OrderStatusEnum.NUMBER_7);
						tmpResult = this.ordiniManager.getPagedOrderListFO(
								codimp, 
								this.model.getCurrentPage(), 
								this.model.getiDisplayLength(),
								filtri);
