...
public class UserActivationAction extends UserRegBaseAction implements IUserActivationAction, SessionAware {
	...
	public String reactivate() {
		...
		evento.setEventType(PortGareEventsConstants.PROCESSA_TOKEN);
		evento.setLevel(Event.Level.INFO);
		evento.setMessage("Utilizzo token riattivazione " + _token);
		try {
			...
			if (username==null || username.length()==0) {
				...
				evento.setLevel(Event.Level.ERROR);
				evento.setDetailMessage("Token non valido, impossibile estrarre lo username");
			} else if (!_userManager.isPasswordNew(username, _password)) {
				addFieldError("password", getText("passwordUserInThePast"));
				target = INPUT;
				evento.setLevel(Event.Level.ERROR);
				evento.setDetailMessage("La password immessa è già stata utilizzata in precedenza");
			} else {
				evento.setUsername(username);
				getUserRegManager().reactivateUser(username, _password, _token);
				getRequest().getSession().setAttribute(JpUserRegSystemConstants.USER_REG_ALREADY_EXECUTED_REACTIVATION, true);
			}
		} catch (Exception e) {
