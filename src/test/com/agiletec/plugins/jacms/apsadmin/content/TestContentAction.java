...
public class TestContentAction extends AbstractBaseTestContentAction {
	...
	public void testValidate_3() throws Throwable { // Description maxlength
		String insertedDescr = "Descrizione che supera la lunghezza massima di cento caratteri; " +
				"Ripeto, Descrizione che supera la lunghezza massima di cento caratteri";
		try {
			String result = this.executeCreateNewVoid("ART", "descr", Content.STATUS_DRAFT, "admin");
			assertEquals(Action.SUCCESS, result);
			Content contentOnSession = this.getContentOnEdit();
			assertNotNull(contentOnSession);
			...
			List<String> mainGroupFieldsErrors = fieldErros.get("mainGroup");
			assertEquals(1, mainGroupFieldsErrors.size());
	...
	public void testValidate_4() throws Throwable {
		String insertedDescr = "XXX Prova Validazione XXX";
		try {
			String result = this.executeCreateNewVoid("RAH", "descr", Content.STATUS_DRAFT, "admin");
			assertEquals(Action.SUCCESS, result);
			Content contentOnSession = this.getContentOnEdit();
			assertNotNull(contentOnSession);
			...
			assertNotNull(attribute);
			assertEquals("CheckBox", attribute.getType());
			assertEquals(false, attribute.getValue());
