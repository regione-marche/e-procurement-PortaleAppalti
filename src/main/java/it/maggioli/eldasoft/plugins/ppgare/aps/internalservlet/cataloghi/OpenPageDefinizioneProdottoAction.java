...
public class OpenPageDefinizioneProdottoAction extends AbstractOpenPageAction implements SessionAware {
    ...
    @Validate(EParamValidation.MARCA_PRODOTTO)
    private String  marcaProdottoProduttore;
    @Validate(EParamValidation.CODICE_PRODOTTO)
    private String  codiceProdottoProduttore;
    @Validate(EParamValidation.NOME_COMMERCIALE)
    private String  nomeCommerciale;
    @Validate(EParamValidation.CODICE_PRODOTTO)
    private String  codiceProdottoFornitore;
    @Validate(EParamValidation.DESCRIZIONE_AGGIUNTIVA)
    private String  descrizioneAggiuntiva;
    @Validate(EParamValidation.DIMENSIONI)
    ...
    public String openPage() {
        ...
        this.session.put(PortGareSystemConstants.SESSION_ID_PAGINA,
                         PortGareSystemConstants.WIZARD_PAGINA_DEFINIZIONE_PRODOTTO);
        ...
        if (null != this.getCurrentUser()
            ...
            try {
                ...
                String tempDir = StrutsUtilities.getTempDir(this.getRequest().getSession().getServletContext(),
                                                            multipartSaveDir).getAbsolutePath();
                ...
                this.setProdottiCaricatiAOE(this.cataloghiManager.getNumProdottiAltriOEInArticolo(
                        ...
                        this.getCurrentUser().getUsername()));
                Double migliorPrezzo =
                        this.cataloghiManager.getPrezzoMiglioreArticolo(prodottoHelper.getArticolo().getIdArticolo());
    ...
    private void fillFieldsProduct(ProdottoType prodotto, WizardArticoloHelper articolo) {
        ...
        this.dataScadenzaOfferta = CalendarValidator.getInstance().format(prodotto.getDataScadenzaOfferta(), "dd/MM" +
                "/yyyy");
        ...
        this.prezzoUnitario =
                BigDecimal.valueOf(new Double(prodotto.getPrezzoUnitario())).setScale(articolo.getDettaglioArticolo().getNumDecimaliDetermPrezzo(), BigDecimal.ROUND_HALF_UP).toPlainString();
    }
