...
<%-- recupera la classifica associata all'utente --%>
<s:set var="posizioneClassifica" value="" />
<s:iterator var="item" value="elencoClassifica" status="stat">
	<s:if test="%{#item.username == #session.currentUser.username}" >
		<s:set var="posizioneClassifica" value="#item.posizione" />
	</s:if>
</s:iterator>
...
<c:choose>
	...
	<c:when test="${tipoClassifica == 2}" >
		<%-- elencoClassifica[...] 
			[0] contiene la posizione dell'utente
			[1] contiene la migliore offerta in assoluto
		--%>
		<div class="detail-row">
			<label><wp:i18n key="LABEL_POSIZIONE_CLASSIFICA" /> : </label>
			<s:property value="#posizioneClassifica" />&deg; <%-- classificato --%>
		</div>
		<div class="detail-row">		
