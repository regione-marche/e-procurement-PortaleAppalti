...
<div class="portgare-view">
	...
	<h2><wp:i18n key="LABEL_BUSTE_TECNICHE" /> : <wp:i18n key="TITLE_PAGE_LISTA_LOTTI" /></h2>
						...
						<td>
							<s:if test="%{#session.aperturaLotto.get(#lotto)}">
								<a 	href='<wp:action path="/ExtStr2/do/FrontEnd/GareTel/openPageDocumenti.action" />&amp;tipoBusta=2&amp;operazione=${operazione}&amp;codice=<s:property value="%{#lotto}"/>&amp;codiceGara=${codiceGara}&amp;progressivoOfferta=${progressivoOfferta}&amp;${tokenHrefParams}'>
									<wp:i18n key="LABEL_LOTTO" /> <s:property value="%{#session.riepilogoBuste.listaCodiciInterniLotti.get(#lotto)}" />&nbsp;-&nbsp;<s:property value="%{#session.riepilogoBuste.busteTecnicheLotti.get(#lotto).oggetto}"/>
								</a>
							</s:if>
							<s:else>
								<!-- lotto DA PROCESSARE -->
								(<wp:i18n key="LABEL_BUSTA_CHIUSA_DA_TRASMETTERE" />) <wp:i18n key="LABEL_LOTTO" /> <s:property value="%{#session.riepilogoBuste.listaCodiciInterniLotti.get(#lotto)}" />&nbsp;-&nbsp;<s:property value="%{#session.riepilogoBuste.busteTecnicheLotti.get(#lotto).oggetto}"/>
							</s:else>
						</td>
						<td class="azioni">
