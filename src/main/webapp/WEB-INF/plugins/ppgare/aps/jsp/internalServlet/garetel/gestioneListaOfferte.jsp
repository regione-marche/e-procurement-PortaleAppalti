...
<div class="portgare-view">
	...
	<fieldset>
		...
		<div class="table-container">
			...
			<caption style="display:none;">${labelLegend}</caption>
				<tr>
					<th scope="col">&nbsp;&nbsp;#&nbsp;&nbsp;</th>
				...
				<s:iterator value="listaOfferte" var="item" status="stat">
					...
					<%-- tipo di pulsante da visualizzare per la riga => "modifica" oppure "visualizza riepilogo" --%>
					<c:set var="showBtnRiepilogo" value="0"/>
					...
					<s:else>
						...
						<c:choose>
							...
							<c:when test="${operazione == inviaOfferta}">
								...
								<c:set var="showBtnRiepilogo" value="1"/>
							</c:when>
							<c:otherwise>
								Page "gestioneListaOfferte": parameter "operazione" not set!
							</c:otherwise>
						...
						<td class="azioni">
							<ul>
								<s:if test="%{#terminiAperti || #item.stato != 1}">
									...
									<wp:i18n key="TITLE_MODIFICA_BUSTA" var="buttonModifica"/>
									<c:set var="clsTipoOperazione" value="modify"/>
									<c:if test="${showBtnRiepilogo != 0}">
										<wp:i18n key="BUTTON_DETTAGLIO_GARA_RIEPILOGO_OFFERTA" var="buttonModifica"/>
										<c:set var="clsTipoOperazione" value="summary"/>
									</c:if>
										...
										<form action="<wp:action path='${href}' />" method="post">
											...
											<c:choose>
												<c:when test="${skin == 'highcontrast' || skin == 'text'}">
													<a href="javascript:;" onclick="parentNode.submit();" title='${buttonModifica}' class="bkg ${clsTipoOperazione}"  >
													</a>
												</c:when>
												<c:otherwise>
													<a href="javascript:;" onclick="parentNode.submit();" title='${buttonModifica}' class="bkg ${clsTipoOperazione}"  >
