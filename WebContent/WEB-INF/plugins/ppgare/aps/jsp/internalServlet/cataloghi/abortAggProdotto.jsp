...
<div class="portgare-view">
	...
	<h2><wp:i18n key='TITLE_PAGE_CATALOGHI_ANNULLA_AGGIORNAMENTO'/></h2>

	<p><wp:i18n key='LABEL_CATALOGHI_ANNULLA_AGGIORNAMENTO_1'/> <c:out value="${prodotto}"/> <wp:i18n key='LABEL_DEL_CATALOGO'/> <c:out value="${catalogo}"/>.</p>
	<p><wp:i18n key='LABEL_CATALOGHI_ANNULLA_AGGIORNAMENTO_2'/></p>
	...
	<div class="azioni">
		<form action="<wp:action path="/ExtStr2/do/FrontEnd/Cataloghi/cancelWizardProdotto.action" />" method="post" class="azione">
			...
			<div>
				...
				<input type="hidden" name="prodottoId" value="<s:property value="prodottoId" />"/>
				<input type="hidden" name="inCarrello" value="<s:property value="inCarrello" />"/>
		...
		<form action="<wp:action path="${href}" />" method="post" class="azione">
			...
			<div>
				<input type="hidden" name="ext" value="${param.ext}" />
				<wp:i18n key="LINK_BACK_TO_UPDATE" var="valueBackToButton" />
