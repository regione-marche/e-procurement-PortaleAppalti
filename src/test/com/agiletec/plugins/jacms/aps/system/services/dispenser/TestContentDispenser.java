...
public class TestContentDispenser extends BaseTestCase {
    ...
    public void testGetRenderedContent_1() throws Throwable {
    	...
    	String output = _contentDispenser.getRenderedContent("ART1", 2, "en", reqCtx);
    	assertEquals(this.replaceNewLine(_attendedEnART1.trim()), this.replaceNewLine(output.trim()));
    	
    	this.setUserOnSession("admin");
    	output = _contentDispenser.getRenderedContent("ART1", 2, "en", reqCtx);
    	assertEquals(this.replaceNewLine(_attendedEnART1.trim()), this.replaceNewLine(output.trim()));
    	output = _contentDispenser.getRenderedContent("ART104", 2, "it", reqCtx);
    	assertEquals(this.replaceNewLine(_attendedItART104.trim()), this.replaceNewLine(output.trim()));
    ...
    public void testGetRenderedContent_2() throws Throwable {
    	...
    	String output = this._contentDispenser.getRenderedContent("ART120", 2, "it", reqCtx);
		assertEquals(this.replaceNewLine(_attendedItART120.trim()), this.replaceNewLine(output.trim()));
    	
    	output = this._contentDispenser.getRenderedContent("ART120", 2, "en", reqCtx);
    	assertEquals(this.replaceNewLine(_attendedEnART120.trim()), this.replaceNewLine(output.trim()));
    	
    	output = this._contentDispenser.getRenderedContent("ART121", 2, "it", reqCtx);
		assertEquals(this.replaceNewLine(_attendedItART121.trim()), this.replaceNewLine(output.trim()));
    	
    	output = this._contentDispenser.getRenderedContent("ART121", 2, "en", reqCtx);
    	assertEquals(this.replaceNewLine(_attendedEnART121.trim()), this.replaceNewLine(output.trim()));
    	
    	output = this._contentDispenser.getRenderedContent("ART122", 2, "en", reqCtx);
    	assertEquals(this.replaceNewLine(_attendedEnART122.trim()), this.replaceNewLine(output.trim()));
    }
    
    public void testGetUnauthorizedContent() throws Throwable {
    	RequestContext reqCtx = this.getRequestContext();
    	
    	String output = _contentDispenser.getRenderedContent("ART104", 2, "it", reqCtx);
    	assertEquals("Current user 'guest' can't view this content", output.trim());
    	
    	this.setUserOnSession("editorCustomers");
    	output = _contentDispenser.getRenderedContent("ART104", 2, "it", reqCtx);
    	assertEquals("Current user 'editorCustomers' can't view this content", output.trim());
    	
    	this.setUserOnSession("supervisorCustomers");
    	output = _contentDispenser.getRenderedContent("ART104", 2, "it", reqCtx);
    	assertEquals("Current user 'supervisorCustomers' can't view this content", output.trim());
    }
