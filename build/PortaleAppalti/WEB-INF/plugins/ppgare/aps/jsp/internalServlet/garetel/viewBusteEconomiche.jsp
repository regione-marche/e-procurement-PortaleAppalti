...
<div class="portgare-view">
	...
	<h2><wp:i18n key="LABEL_BUSTE_ECONOMICHE" /> : <wp:i18n key="TITLE_PAGE_LISTA_LOTTI" /></h2>
	...
	<fieldset>
		...
		<table>
			...
			<s:iterator value="%{#session.dettPartecipGara.lotti}" var="lotto" status="stat">
							...
							<s:if test="%{#session.aperturaLotto.get(#lotto)}">
								<s:if test="%{#session.wizardOfferta.get(#lotto)}">
									...
									<a href='<wp:action path="${href}"/>&amp;tipoBusta=3&amp;operazione=${operazione}&amp;codice=<s:property value="%{#lotto}"/>&amp;codiceGara=${codiceGara}&amp;progressivoOfferta=${progressivoOfferta}&amp;${tokenHrefParams}'>
										<wp:i18n key="LABEL_LOTTO" /> <s:property value="%{#session.riepilogoBuste.listaCodiciInterniLotti.get(#lotto)}" /> - 
										<s:property value="%{#session.riepilogoBuste.busteEconomicheLotti.get(#lotto).oggetto}"/>
									</a>
								</s:if>
								<s:else>
									<c:set var="href" value="/ExtStr2/do/FrontEnd/GareTel/openPageDocumenti.action"/>
									<a href='<wp:action path="${href}" />&amp;tipoBusta=3&amp;operazione=${operazione}&amp;codice=<s:property value="%{#lotto}"/>&amp;codiceGara=${codiceGara}&amp;progressivoOfferta=${progressivoOfferta}&amp;${tokenHrefParams}'>
										<wp:i18n key="LABEL_LOTTO" /> <s:property value="%{#session.riepilogoBuste.listaCodiciInterniLotti.get(#lotto)}" />&nbsp;-&nbsp;<s:property value="%{#session.riepilogoBuste.busteEconomicheLotti.get(#lotto).oggetto}"/>
									</a>
								</s:else>
							</s:if>
							<s:else>
								<!-- lotto DA PROCESSARE -->
								(<wp:i18n key="LABEL_BUSTA_CHIUSA_DA_TRASMETTERE" />) <wp:i18n key="LABEL_LOTTO" /> <s:property value="%{#session.riepilogoBuste.listaCodiciInterniLotti.get(#lotto)}" />&nbsp;-&nbsp;<s:property value="%{#session.riepilogoBuste.busteEconomicheLotti.get(#lotto).oggetto}"/> 
							</s:else>
						</td>
						<td class="azioni">
