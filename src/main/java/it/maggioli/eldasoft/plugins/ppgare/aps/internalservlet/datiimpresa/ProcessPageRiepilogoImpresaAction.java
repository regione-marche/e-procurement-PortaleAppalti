			    ...
			    if (!helper.getXmlDocumentAggiornamentoAnagrafica().validate(validationOptions)) {
					...
					try {
						...
						if (esito.isEsitoOk()) {
							...
							// Non si pu� cancellare perch� utilizzata in CancelAggiornamentoAction.backAfter in caso di iscr_albo/asta/rich_part_bando
		...
		if (helper == null) {
	    	...
	    	try {
				...
				ApsSystemUtils.logThrowable(e, this, "next",
						"Errore durante la ricerca delle propriet� di visualizzazione dello step dati ulteriori");
			}
