...
<c:if test="${param.nascondiSoccorsoIstruttorio eq '1'}">
	<s:set var="mostraSoccorsoIstruttorio" value="false"/>
</c:if>
...
<div class="detail-section last-detail-section">
	...
	<div class="detail-row">
		...
		<ul>
			...
			<li>
				<a href='<wp:action path="/ExtStr2/do/FrontEnd/Comunicazioni/initNuovaComunicazione.action"/>&amp;codice=<s:property value="codice"/>&amp;namespace=<s:property value="%{namespace}"/>&amp;actionName=<s:property value="%{actionName}"/>&amp;${tokenHrefParams}'>
	...
	<c:if test="${ mostraRichiesteInviate }">
		<c:if test="${ stato != 1 && sessionScope.currentUser != 'guest' }">
			<h3 class="detail-section-title">
			...
			<div class="detail-row">
				<ul>
					<li>
						<a href='<wp:action path="/ExtStr2/do/FrontEnd/Comunicazioni/elencoRichieste.action" />&amp;codiceElenco=${dettaglio.datiGeneraliBandoIscrizione.codice}&amp;${tokenHrefParams}' 
