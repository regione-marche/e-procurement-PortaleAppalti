...
public class ImpresaAction extends EncodedDataAction implements SessionAware, IDownloadAction {
		...
		if (null != this.getCurrentUser()
			...
			try {				
				WizardDatiImpresaHelper datiImpresaHelper = ImpresaAction
						.getLatestDatiImpresa(this.getCurrentUser().getUsername(), this, this.appParamManager);

				if (datiImpresaHelper != null) {
					this.session.put(
							PortGareSystemConstants.SESSION_ID_DETT_ANAGRAFICA_IMPRESA,
							datiImpresaHelper);
					this.session.put(
							PortGareSystemConstants.SESSION_ID_URL_RITORNO_AGG_ANAGRAFICA_IMPRESA,
							this.fromUrl);
					this.session.put(
							PortGareSystemConstants.SESSION_ID_MODULO_RITORNO_AGG_ANAGRAFICA_IMPRESA,
							this.fromModule);
					this.session.put(
							PortGareSystemConstants.SESSION_ID_ACTION_RITORNO_AGG_ANAGRAFICA_IMPRESA,
							this.fromAction);
					this.session.put(
							PortGareSystemConstants.SESSION_ID_FRAME_RITORNO_AGG_ANAGRAFICA_IMPRESA,
							this.fromFrame);
				}
	 ...
	 * assenza di comunicazioni, direttamente dalle tabelle del backoffice. I
	 * dati estratti vengono posti nell'helper in uscita.<br/>
	...
	public static WizardRegistrazioneImpresaHelper getLatestDatiImpresa(
			...
			IAppParamManager appParamManager) throws ApsException, XmlException 
	{
		...
		if (comunicazioni == null || comunicazioni.size() == 0) {			
			...
			for (int i = 0; i < datiImpresa.getDatiImpresa().sizeOfLegaleRappresentanteArray(); i++) {
				ISoggettoImpresa legaleRappresentante =new SoggettoImpresaHelper(
						datiImpresa.getDatiImpresa().getLegaleRappresentanteArray()[i],
						CommonSystemConstants.TIPO_SOGGETTO_LEGALE_RAPPRESENTANTE);
				if(legaleRappresentante.getDataFineIncarico() !=null){
					datiImpresa.getDatiImpresa().getLegaleRappresentanteArray()[i].setSolaLettura(true);
				}else{
					datiImpresa.getDatiImpresa().getLegaleRappresentanteArray()[i].setSolaLettura(false);
				}
			}
			
			for (int i = 0; i < datiImpresa.getDatiImpresa().sizeOfDirettoreTecnicoArray(); i++) {
				ISoggettoImpresa direttoreTecnico =new SoggettoImpresaHelper(
						datiImpresa.getDatiImpresa().getDirettoreTecnicoArray()[i],
						CommonSystemConstants.TIPO_SOGGETTO_DIRETTORE_TECNICO);
				if(direttoreTecnico.getDataFineIncarico() !=null){
					datiImpresa.getDatiImpresa().getDirettoreTecnicoArray()[i].setSolaLettura(true);
				}else{
					datiImpresa.getDatiImpresa().getDirettoreTecnicoArray()[i].setSolaLettura(false);
				}
			}
			
			for (int i = 0; i < datiImpresa.getDatiImpresa().sizeOfAltraCaricaArray(); i++) {
				ISoggettoImpresa altraCarica =new SoggettoImpresaHelper(
						datiImpresa.getDatiImpresa().getAltraCaricaArray()[i],
						CommonSystemConstants.TIPO_SOGGETTO_ALTRA_CARICA);
				if(altraCarica.getDataFineIncarico() !=null){
					datiImpresa.getDatiImpresa().getAltraCaricaArray()[i].setSolaLettura(true);
				}else{
					datiImpresa.getDatiImpresa().getAltraCaricaArray()[i].setSolaLettura(false);
				}
			}
			
			for (int i = 0; i < datiImpresa.getDatiImpresa().sizeOfCollaboratoreArray(); i++) {
				ISoggettoImpresa collaboratore =new SoggettoImpresaHelper(
						datiImpresa.getDatiImpresa().getCollaboratoreArray()[i],
						CommonSystemConstants.TIPO_SOGGETTO_COLLABORATORE);
				if(collaboratore.getDataFineIncarico() !=null){
					datiImpresa.getDatiImpresa().getCollaboratoreArray()[i].setSolaLettura(true);
				}else{
					datiImpresa.getDatiImpresa().getCollaboratoreArray()[i].setSolaLettura(false);
				}		
			}
			...
			if (allegatoAggiornamentoAnagrafica == null) {
				...
				if(action != null) {
					action.addActionError(action
						.getText("Errors.aggiornamentoAnagrafica.xmlAggiornamentoNotFound"));
		...
		if (helper != null) {
			helper.setDittaIndividuale(action
					.getMaps()
					.get(InterceptorEncodedData.LISTA_TIPI_IMPRESA_DITTA_INDIVIDUALE)
					.containsKey(
							helper.getDatiPrincipaliImpresa().getTipoImpresa()));
			helper.setLiberoProfessionista(action
					.getMaps()
					.get(InterceptorEncodedData.LISTA_TIPI_IMPRESA_LIBERO_PROFESSIONISTA)
					.containsKey(
							helper.getDatiPrincipaliImpresa().getTipoImpresa()));
			helper.setConsorzio(action
					.getMaps()
					.get(InterceptorEncodedData.LISTA_TIPI_IMPRESA_CONSORZIO)
					.containsKey(
							helper.getDatiPrincipaliImpresa().getTipoImpresa()));
			helper.setMailUtentePrecedente(DatiImpresaChecker
					.getEmailRiferimento(helper.getDatiPrincipaliImpresa()
