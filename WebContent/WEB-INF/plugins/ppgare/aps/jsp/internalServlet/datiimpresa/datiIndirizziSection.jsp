...
<s:if test="%{delete}">
	<s:set var="ind"  value="%{#session[#sessionId].indirizziImpresa.get(idDelete)}"/>
	<p class="question">
...
<s:else>
	<s:if test="%{#session[#sessionId].indirizziImpresa.size() > 0}">
		...
		<fieldset>
			...
			<table class="wizard-table" summary="<wp:i18n key="ELENCO_INDIRIZZI_TABELLA_SUMMARY" />">
				...
				<s:iterator value="#session[#sessionId].indirizziImpresa" status="status">
					<tr>
						<td>
