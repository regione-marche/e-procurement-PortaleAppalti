...
public class TestPageConfigAction extends ApsAdminBaseTestCase {
	...
	public void testJoinShowlet() throws Throwable {
		...
		try {
			...
			_pageManager.updatePage(pagina_1);
		}
	}
	...
	private void testJoinRemoveShowlet(String showletTypeCode) throws Throwable {
		...
		try {
			...
			result = this.executeRemoveShowlet(pageCode, frame, "admin");
			assertEquals(Action.SUCCESS, result);
			pagina_1 = this._pageManager.getPage(pageCode);
			assertNull(pagina_1.getShowlets()[frame]);
		} catch (Throwable t) {
	...
	public void testRemoveShowlet() throws Throwable {
		...
		try {
			...
			String result = this.executeRemoveShowlet(pageCode, frame, "pageManagerCoach");
			assertEquals("pageTree", result);
			assertEquals(1, this.getAction().getActionErrors().size());
			result = this.executeRemoveShowlet(pageCode, frame, "admin");
			assertEquals(Action.SUCCESS, result);
			...
			_pageManager.updatePage(contentview);
		}
	}
	
	private String executeRemoveShowlet(String pageCode, int frame, String username) throws Throwable {
		this.setUserOnSession(username);
		this.initAction("/do/Page", "removeShowlet");
		this.addParameter("pageCode", pageCode);
		this.addParameter("frame", String.valueOf(frame));
		return this.executeAction();
	}
