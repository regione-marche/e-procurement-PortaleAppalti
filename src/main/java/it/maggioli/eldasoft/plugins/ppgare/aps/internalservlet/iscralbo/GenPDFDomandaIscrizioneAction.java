	...
	protected boolean reportInit() {
		...
		try {
			if (helper == null) {
				...
				this.setTarget(INPUT);
				continua = false;
			} else if (this.customConfigManager.isVisible("ISCRALBO-DOCUM", "NUMSERIEBOLLODOMANDA") 
					&& ((StringUtils.isEmpty(serialNumberMarcaBollo) || !serialNumberMarcaBollo.matches("^[0-9]{14}")) &&
							(this.customConfigManager.isMandatory("ISCRALBO-DOCUM", "NUMSERIEBOLLODOMANDA") 
									|| !"n.d.".equalsIgnoreCase(this.serialNumberMarcaBollo)))) {
				// la marca da bollo deve essere inserita e composta esattamente da 14 cifre
				session.put(OpenPageScaricaDomandaIscrizioneAction.NOT_VALID_STAMP, this.getText("Errors.wrongIUBD"));
//				addFieldError("serialNumberMarcaBollo", getText("Errors.wrongIUBD"));
				this.setTarget(INPUT);
				continua = false;
			} else {
