...
<c:set var="obblIscrizione" value="${obblIscrizione  && (sessionScope[param.sessionIdObj].datiPrincipaliImpresa.naturaGiuridica == 7)}" />
...
<s:if test="%{delete}">
	<p class="question">
		...
		<s:if test="%{tipoSoggettoDelete == 1}">
			<s:set var="sog" value="%{#session[#sessionId].legaliRappresentantiImpresa.get(idDelete)}"/>
		</s:if>
		<s:elseif test="%{tipoSoggettoDelete == 2}">
			<s:set var="sog" value="%{#session[#sessionId].direttoriTecniciImpresa.get(idDelete)}"/>
		</s:elseif>
		<s:elseif test="%{tipoSoggettoDelete == 3}">
			<s:set var="sog" value="%{#session[#sessionId].altreCaricheImpresa.get(idDelete)}"/>
		</s:elseif>
		<s:else>
			<s:set var="sog" value="%{#session[#sessionId].collaboratoriImpresa.get(idDelete)}"/>
		</s:else>
...
<s:else>
	<s:if test="%{#session[#sessionId].legaliRappresentantiImpresa.size() > 0 
				|| #session[#sessionId].direttoriTecniciImpresa.size() > 0 
				|| #session[#sessionId].altreCaricheImpresa.size() > 0 
				|| #session[#sessionId].collaboratoriImpresa.size() > 0}">
		...
		<fieldset>
			...
			<table class="wizard-table" summary="<wp:i18n key="ELENCO_SOGGETTI_TABELLA_SUMMARY" />">
				...
				<s:set var="elencoSoggetti" value="%{#session[#sessionId].legaliRappresentantiImpresa}"/>
				<s:include value="/WEB-INF/plugins/ppgare/aps/jsp/internalServlet/datiimpresa/inc/iteratorSoggetti.jsp" />
				<s:set var="elencoSoggetti" value="%{#session[#sessionId].direttoriTecniciImpresa}"/>
				<s:include value="/WEB-INF/plugins/ppgare/aps/jsp/internalServlet/datiimpresa/inc/iteratorSoggetti.jsp" />
				<s:set var="elencoSoggetti" value="%{#session[#sessionId].altreCaricheImpresa}"/>
				<s:include value="/WEB-INF/plugins/ppgare/aps/jsp/internalServlet/datiimpresa/inc/iteratorSoggetti.jsp" />
				<s:set var="elencoSoggetti" value="%{#session[#sessionId].collaboratoriImpresa}"/>
				<s:include value="/WEB-INF/plugins/ppgare/aps/jsp/internalServlet/datiimpresa/inc/iteratorSoggetti.jsp" />
			</table>
		</fieldset>
	</s:if>
					...
					<c:choose>
						<c:when test="${empty id}">
							...
							<s:select list="maps['tipiSoggetto']" name="soggettoQualifica" id="soggettoQualifica" value="{soggettoQualifica}"
								...
								<s:optgroup label="%{#attr.labelAltreCollaboratori}" list="maps['tipiCollaborazione']"></s:optgroup>
							</s:select>
						</c:when>
						<c:otherwise>
							<s:iterator value="maps['tipiSoggetto']">
			...
			<div class="element">
				<div class="contents-group">
					...
					<s:textfield name="indirizzo" id="indirizzo" value="%{indirizzo}" 
											 size="40" maxlength="60"  readonly="%{readonlyIndirizzo}" cssClass="%{#classBloccoCampo}" 
											 aria-required="true" />
					<s:set var="classBloccoCampo" value="" />
				...
				<div class="contents-group">
					<s:set var="classBloccoCampo" value="" />
					<label for="provincia" class="provincia"><wp:i18n key="LABEL_PROVINCIA" /> : <span id="obbCap" class="required-field">*</span></label>
