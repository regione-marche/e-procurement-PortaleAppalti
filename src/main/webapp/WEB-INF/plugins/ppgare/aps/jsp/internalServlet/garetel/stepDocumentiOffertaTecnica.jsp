												...
												<s:if test="%{#idDoc == #documento.id && #documenti.docRichiestiVisibile.get(#statusCaricato.index)}">
													...
													<c:choose>
														<c:when test="${skin == 'highcontrast' || skin == 'text'}">
															...
															<li>
																<a href='<wp:action path="/ExtStr2/do/FrontEnd/GareTel/confirmDeleteOffTecAllegatoRichiesto.action"/>&amp;id=${statusCaricato.index}&amp;ext=${param.ext}&amp;tipoBusta=<s:property value="%{#documenti.tipoBusta}"/>&amp;codice=<s:property value="%{#documenti.codice}"/>' 
										...
										<ul>
											...
											<c:choose>
												<c:when test="${skin == 'highcontrast' || skin == 'text'}">
													...
													<li>
														<a href='<wp:action path="/ExtStr2/do/FrontEnd/GareTel/confirmDeleteOffTecAllegatoUlteriore.action"/>&amp;id=${status.index}&amp;ext=${param.ext}&amp;tipoBusta=<s:property value="%{#documenti.tipoBusta}"/>&amp;codice=<s:property value="%{#documenti.codice}"/>' 
														   title='<s:property value="%{#attr.titleEliminaAllegato}"/>' >
															<wp:i18n key="LABEL_ELIMINA_ALLEGATO" />
														</a>
													</li>
