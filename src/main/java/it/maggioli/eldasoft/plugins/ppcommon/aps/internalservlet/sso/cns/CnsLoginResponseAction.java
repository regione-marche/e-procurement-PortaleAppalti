package it.maggioli.eldasoft.plugins.ppcommon.aps.internalservlet.sso.cns;
...
public class CnsLoginResponseAction extends BaseResponseAction implements ServletResponseAware {
	...
	public String prepareLogin() {
		...
		this.urlLogin = prepareCallbackLogin(
				"/do/SSO/CnsLoginResponse.action",
                this.configManager,
			...
			WSAuthServiceSPIDWrapper wsAuthServiceSPID) 
	{
		...
		try {
			...
			.getConfigurationValue(AppParamManager.CNS_WS_AUTHSERVICESCNS_URL);
			String authSystem = CNS_AUTHSYSTEM_DEFAULT;
			...
			if (StringUtils.isEmpty(url)) {
				action.addActionError(action
						.getText("Errors.sso.parameterConfiguration",
								 new String[] { AppParamManager.CNS_WS_AUTHSERVICESCNS_URL }));
				ApsSystemUtils.getLogger()
						.error("prepareLogin(): "
								+ action.getTextFromDefaultLocale(
										"Errors.sso.parameterConfiguration",
										AppParamManager.CNS_WS_AUTHSERVICESCNS_URL));
			...
			action.addActionError(action.getText("Errors.sso.configuration", new String[] { "Maggioli CNS" }));
			action.getRequest().getSession().setAttribute("ACTION_OBJECT", action);
