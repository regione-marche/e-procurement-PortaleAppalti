...
<%--
								...
								<s:if test="%{#doc.iddocdig == null}">
									<td colspan="2">
										<form action="<wp:action path='/ExtStr2/do/FrontEnd/Contratti/processPageDocumentiContratti.action' />"
											...
											<input type="file" name="docToUpload" aria-label="${nome}"
		...
		<s:if test="%{documentiUpload.size > 0}">
			<div class="azioni">
				<form action="<wp:action path="/ExtStr2/do/FrontEnd/Contratti/confermaDialogInvio.action" />" method="post" class="azione">
					<jsp:include page="/WEB-INF/plugins/ppcommon/aps/jsp/token_input.jsp" />
					<div>
						<input type="submit" value="<wp:i18n key="BUTTON_CONFIRM_STIPULE" />" class="button" title="<wp:i18n key="BUTTON_CONFIRM_STIPULE" />">
						<input type="hidden" name="codiceStipula" value="<s:property value="%{codiceStipula}"/>"/>
						<input type="hidden" name="codice" value="<s:property value="%{codice}"/>"/>
						<input type="hidden" name="statoStipula" value="<s:property value="%{statoStipula}"/>"/>
					</div>
				</form>
			</div>
		</s:if>
	...
	<div class="back-link">
		<a href='<wp:action path="/ExtStr2/do/FrontEnd/Contratti/dettaglioStipulaContratti.action" />&amp;id=<s:property value="codice"/>&amp;codice=<s:property value="%{codiceStipula}"/>'
