...
<div class="portgare-view">
	...
	<form action="<wp:action path="/ExtStr2/do/FrontEnd/Aste/rilancio.action" />" method="post" >
		...
		<fieldset>
			...
			<div class="fieldset-row">
				...
				<div class="element">
					...
					<s:else>
						<%-- il campo importo/ribasso non è editabile --%>
		...
		<div class="azioni">
			<s:if test="%{#rilancioPrezziUnitari}" >
				...
				<s:submit value="%{#attr.valuePreviousButton}" title="%{#attr.titlePreviousButton}" cssClass="button" action="backPrezziUnitari" />
			</s:if>
			
			<wp:i18n key="BUTTON_RILANCIA" var="valueRilanciaButton" />
			<s:submit value="%{#attr.valueRilanciaButton}" title="%{#attr.valueRilanciaButton}" cssClass="button" action="rilancio" />
			
			<wp:i18n key="BUTTON_WIZARD_CANCEL" var="valueCancelButton" />
			<wp:i18n key="TITLE_WIZARD_CANCEL" var="titleCancelButton" />
			<s:submit value="%{#attr.valueCancelButton}" title="%{#attr.titleCancelButton}" cssClass="button" action="annullaRilancio" />
